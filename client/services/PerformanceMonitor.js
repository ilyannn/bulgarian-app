...
class PerformanceMonitor {
  ...
  finalizeSession() {
    ...
    const e2e = this.currentSession.timestamps.end - this.currentSession.timestamps.start;
  ...
  getStatistics() {
    ...
    for (const [type, values] of Object.entries(this.metrics)) {
      ...
      const recentAvg =
        values.slice(-recentCount).reduce((acc, v) => acc + v.value, 0) / recentCount;
      const olderAvg =
  ...
  getSessionBreakdown() {
    ...
    if (session.timestamps.start && session.timestamps.end) {
      breakdown.total = Math.round(session.timestamps.end - session.timestamps.start);
  ...
  generateRecommendations(stats) {
    ...
    if (stats.asr.avg > 800) {
      recommendations.push('ASR is slow - consider optimizing beam_size or using a smaller model');
    }

    if (stats.llm.avg > 1000) {
      recommendations.push('LLM response time is high - consider caching common responses');
    }

    if (stats.tts.avg > 500) {
      recommendations.push('TTS generation is slow - consider pre-generating common phrases');
    }

    if (stats.websocket.avg > 100) {
      recommendations.push('WebSocket latency is high - check network conditions');
    }

    if (stats.e2e.trend === 'increasing') {
      recommendations.push('Performance is degrading over time - check for memory leaks');
    }
