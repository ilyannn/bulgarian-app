...
class MiniLessonService {
  ...
  async getMiniLesson(lessonId) {
    try {
      ...
      const response = await fetch(`${this.baseURL}/content/mini-lessons/${lessonId}`);
      if (!response.ok) {
  ...
  async getLessonsForError(errorPattern) {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch lessons for error: ${response.status}`);
      }
  ...
  async shouldTriggerLesson(recentErrors, lessonId) {
    try {
      ...
      const matchingErrors = recentErrors.filter((errorData) => {
        ...
        return errorPatterns.some((pattern) => {
          try {
            ...
            console.warn('Invalid regex pattern in mini-lesson:', pattern, error);
            return false;
          }
        });
      });
      ...
      const avgConfidence =
        matchingErrors.reduce((sum, error) => sum + (error.confidence || 1), 0) /
  ...
  updateLessonProgress(lessonId, completionData) {
    try {
      ...
      data.miniLessonProgress[lessonId] = {
        ...
        timeSpentMs,
        drillResults,
        totalAttempts: (data.miniLessonProgress[lessonId]?.totalAttempts || 0) + 1,
      };
  ...
  getLessonStatistics() {
    ...
    const totalAccuracy = lessonProgress.reduce((sum, lesson) => sum + lesson.accuracyScore, 0);

    const totalTime = lessonProgress.reduce((sum, lesson) => sum + lesson.timeSpentMs, 0);
  ...
  extractLessonProgress(progressData) {
    ...
    for (const [grammarId, progress] of Object.entries(progressData.progress)) {
      if (grammarId.includes('mini_lesson') || progress.drillType === 'mini_lesson') {
    ...
    if (progressData.miniLessonProgress) {
      for (const [lessonId, lessonData] of Object.entries(progressData.miniLessonProgress)) {
  ...
  async getNewUserLessons(limit = 3) {
    try {
      ...
      const beginnerLessons = allLessons.filter((lesson) => lesson.level?.includes('A2'));
