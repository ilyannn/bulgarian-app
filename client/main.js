...
class BulgarianVoiceCoach {
	...
	addCoachResponse(payload) {
		...
		if (payload.corrections && payload.corrections.length > 0) {
			...
			html +=
				'<div style="margin-top: 0.5rem; font-weight: 500;">Corrections:</div>';
		...
		if (payload.drills && payload.drills.length > 0) {
			...
			html +=
				'<div style="font-weight: 500; margin-bottom: 0.5rem;">Practice Drills:</div>';
	...
	cleanupBlobUrls() {
		...
		if (
			...
			this.audioPlayer.src.startsWith("blob:")
		) {
...
window.addEventListener("blur", () => {
	...
	if (window.voiceCoach && window.voiceCoach.isRecording) {
