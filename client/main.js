...
class BulgarianVoiceCoach {
	constructor() {
		...
		this.voiceProfileSelect = document.getElementById("voice-profile");
		this.currentVoiceProfile =
			localStorage.getItem("voiceProfile") || "natural";
	...
	/**
	 * Safely escape HTML content to prevent DOM-based XSS attacks
	...
	initializeEventListeners() {
		...
		if (this.voiceProfileSelect) {
			...
			this.voiceProfileSelect.value = this.currentVoiceProfile;

			// Handle profile changes
			this.voiceProfileSelect.addEventListener("change", (e) => {
				this.currentVoiceProfile = e.target.value;
				localStorage.setItem("voiceProfile", this.currentVoiceProfile);
				this.showSuccess(
					`Voice changed to: ${this.getProfileDescription(this.currentVoiceProfile)}`,
				);
			});

			// Load profile descriptions from API
	...
	async initializeAudio() {
		try {
			...
			try {
				...
				this.showError(
					"Audio system unavailable. Voice features will not work.",
				);
			}
		}
	}
	...
	updateAudioLevel(level) {
		...
		const smoothedLevel =
			this.audioLevelHistory.reduce((a, b) => a + b, 0) /
			this.audioLevelHistory.length;
		...
		if (percentage < 20) {
			this.micLevelBar.style.background =
	...
	addFinalTranscript(text) {
		...
		this.transcriptDisplay.addFinalTranscript(
			text,
			confidence,
			this.lastDetectedErrors,
		);
	...
	async playLastResponse() {
		...
		try {
			...
			const ttsUrl = `/tts?text=${encodeURIComponent(this.lastResponseText)}&profile=${this.currentVoiceProfile}`;
			const response = await window.fetch(ttsUrl);
	...
	updateLatencyDisplay() {
		...
		if (breakdown.stages) {
			const details = [];
			if (breakdown.stages.vad)
				details.push(`VAD: ${breakdown.stages.vad.duration}ms`);
			if (breakdown.stages.asr)
				details.push(`ASR: ${breakdown.stages.asr.duration}ms`);
			if (breakdown.stages.llm)
				details.push(`LLM: ${breakdown.stages.llm.duration}ms`);
			if (breakdown.stages.tts)
				details.push(`TTS: ${breakdown.stages.tts.duration}ms`);
	...
	showError(message, duration = 5000) {
		...
		closeBtn.onclick = () => toast.remove(); // Safe event handler
	...
	async loadVoiceProfiles() {
		try {
			const response = await fetch("/tts/profiles");
			if (response.ok) {
				const data = await response.json();
				this.voiceProfiles = data.available_profiles;

				// Update select options with descriptions
				if (this.voiceProfileSelect) {
					Array.from(this.voiceProfileSelect.options).forEach((option) => {
						const description = this.voiceProfiles[option.value];
						if (description) {
							option.title = description;
							// Optionally update the text to include description
							option.text = `${option.value.charAt(0).toUpperCase() + option.value.slice(1)} - ${description}`;
						}
					});
				}
			}
	...
	getProfileDescription(profile) {
		if (this.voiceProfiles?.[profile]) {
			return this.voiceProfiles[profile];
		}
		// Default descriptions if API not loaded yet
		const defaults = {
			natural: "More natural-sounding Bulgarian voice",
			standard: "Standard Bulgarian voice",
	...
	handleOnlineStatus() {
		if (navigator.onLine) {
			...
			this.showError(
				"You appear to be offline. Voice features may not work properly.",
			);
			this.isConnected = false;
	...
	handleVisibilityChange() {
		if (document.hidden) {
			...
			if (this.wasRecordingBeforeHidden) {
				...
				this.showInfo(
					"Page visible again. Click microphone to resume recording.",
				);
			}
		}
	}
    ...
    `;
		...
		const practiceButtons = this.transcriptArea.querySelectorAll(
			".warmup-practice-btn",
		);
	...
	async startGrammarPractice(grammarId) {
		try {
			...
			const response = await window.fetch(
				`/content/drills/${encodeURIComponent(grammarId)}`,
			);
			if (!response.ok) {
			...
			if (drills.length > 0) {
				...
				// Create practice drill container safely
				const container = document.createElement("div");
				container.className = "practice-drill";
				container.style.cssText =
					"background: rgba(255, 193, 7, 0.1); border-radius: 12px; padding: 2rem; text-align: center;";

				// Create header
				const header = document.createElement("h3");
				header.style.cssText = "color: #ffc107; margin-bottom: 1rem;";
				header.textContent = `⚡ Quick Practice: ${grammarId}`;
				container.appendChild(header);

				// Create drill content box
				...
				promptP.style.cssText =
					"font-weight: 500; margin-bottom: 1rem; color: #333;";
				promptP.textContent = drill.prompt;
				contentBox.appendChild(promptP);

				// Create answer
				const answerDiv = document.createElement("div");
				answerDiv.style.cssText =
					"color: #28a745; font-weight: 600; font-size: 1.1rem;";
				answerDiv.textContent = `Answer: "${drill.expected_answer}"`;
				contentBox.appendChild(answerDiv);
				...
				closeBtn.addEventListener("click", () => {
					...
					defaultDiv.textContent = "Start speaking to practice Bulgarian...";
					this.transcriptArea.innerHTML = "";
					this.transcriptArea.appendChild(defaultDiv);
				});
				container.appendChild(closeBtn);

				// Replace content safely
				this.transcriptArea.innerHTML = "";
				this.transcriptArea.appendChild(container);
			} else {
				this.showToast(
	...
	async loadAudio() {
		...
		try {
			...
			this.audioPlayer.onError((error) => {
				this.showError(
					`Audio playback error: ${error.message || "Unknown error"}`,
				);
			});
