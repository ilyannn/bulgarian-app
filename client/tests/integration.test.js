...
describe("Voice Coach Integration Tests", () => {
	...
	describe("Complete Voice Interaction Workflow", () => {
		it("should handle end-to-end voice interaction successfully", async () => {
			...
			expect(mockSetup.mockElements.connectionText.textContent).toBe(
				"Connected",
			);
			...
			expect(
				mockSetup.mockElements.transcriptArea.appendChild,
			).toHaveBeenCalled();

			// Verify final state
			expect(voiceCoach.lastResponseText).toBe(
				"Отлично произношение! Много добре казано.",
			);
			expect(mockSetup.mockElements.playLastBtn.disabled).toBe(false);
			expect(mockSetup.mockElements.latencyText.textContent).toMatch(
				/Latency: \d+ms/,
			);
		});
	...
	describe("TTS Integration", () => {
		...
		it("should handle TTS playback successfully", async () => {
			...
			expect(mockSetup.mockElements.playLastBtn.textContent).toBe(
				"🔄 Loading...",
			);
		});
