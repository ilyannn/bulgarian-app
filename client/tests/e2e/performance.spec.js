...
test.describe('Performance E2E Tests', () => {
  test('should handle rapid microphone button clicks', async ({ page }) => {
    ...
    await expect(page.locator('#mic-status')).toContainText(/Click microphone|Recording/);
  });
  ...
  test('should handle memory stress with many transcript entries', async ({ page }) => {
    await page.goto('/');
    ...
    await page.evaluate(() => {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        line.className = i % 2 === 0 ? 'transcript-line final' : 'transcript-line coach';
    ...
    await expect(page.locator('.transcript-line')).toHaveCount(1000, { timeout: 10000 });

    // Clear should still work
    await page.locator('#clear-btn').click();
    await expect(page.locator('.transcript-line')).toHaveCount(0);
    await expect(page.locator('#transcript-area')).toContainText('Start speaking');
  });
  ...
  test('should handle WebSocket message flooding', async ({ page }) => {
    ...
    await expect(page.locator('.transcript-line.final')).toContainText('Flood message 49');
  });

  test('should maintain performance during visual updates', async ({ page }) => {
    await page.addInitScript(() => {
...
test.describe('Edge Case E2E Tests', () => {
  ...
  test('should handle extremely long text inputs', async ({ page }) => {
    ...
    await expect(page.locator('.transcript-line.final')).toBeVisible({ timeout: 3000 });
  ...
  test('should recover from audio context suspension', async ({ page }) => {
    ...
    await expect(page.locator('#mic-status')).toHaveText('Recording...', { timeout: 5000 });
  });
});
