...
// Enhanced mock for document with proper DOM element methods
const createMockElement = () => {
	const element = {
		...
		querySelector: (selector) => {
			// Return another mock element when querySelector is called
			return {
				textContent: "",
				innerHTML: "",
				className: "",
...
global.document = {
	...
	querySelectorAll: () => [],
	createElement: (tag) => {
		// Always return a new mock element with all methods
...
class BulgarianVoiceCoach {
	...
	updatePartialTranscript(text) {
		...
		partialLine.innerHTML =
			'<strong>You (partial):</strong> <span class="bg-text"></span>';
	...
	async playLastResponse() {
		...
		try {
			...
			const response = await fetch(
				`/tts?text=${encodeURIComponent(this.lastResponseText)}`,
			);
...
describe("BulgarianVoiceCoach", () => {
	...
	describe("Recording Control", () => {
		...
		it("should stop recording", () => {
			...
			expect(mockElements.micStatus.textContent).toBe(
	...
	describe("Status Updates", () => {
		it("should update connection status to connected", () => {
			...
			expect(mockElements.connectionStatus.classList.add).toHaveBeenCalledWith(
				"connected",
			);
			expect(mockElements.connectionText.textContent).toBe("Connected");
		});
		...
		it("should update audio status to recording", () => {
			...
			expect(mockElements.audioStatus.classList.add).toHaveBeenCalledWith(
				"connected",
			);
	...
	describe("TTS Playback", () => {
		...
		it("should handle TTS request failure", async () => {
			...
			expect(mockElements.playLastBtn.disabled).toBe(false);
			expect(mockElements.playLastBtn.textContent).toBe(
				"🔊 Play Last Response",
			);
		});
