...
describe('EnhancedCorrectionsSystem', () => {
  ...
  describe('Correction Processing', () => {
    ...
    it('should store corrections in internal map', () => {
      ...
      const correctionEntries = Array.from(correctionsSystem.corrections.values());
  ...
  describe('Severity Determination', () => {
    it('should assign high severity to grammar/syntax errors', () => {
      expect(correctionsSystem.determineSeverity({ type: 'grammar' })).toBe('high');
      expect(correctionsSystem.determineSeverity({ type: 'syntax' })).toBe('high');
      expect(correctionsSystem.determineSeverity({ type: 'verb_conjugation' })).toBe('high');
    });

    it('should assign medium severity to vocabulary/preposition errors', () => {
      expect(correctionsSystem.determineSeverity({ type: 'vocabulary' })).toBe('medium');
      expect(correctionsSystem.determineSeverity({ type: 'preposition' })).toBe('medium');
      expect(correctionsSystem.determineSeverity({ type: 'case' })).toBe('medium');
    });

    it('should assign low severity to other error types', () => {
      expect(correctionsSystem.determineSeverity({ type: 'pronunciation' })).toBe('low');
      expect(correctionsSystem.determineSeverity({ type: 'adjective_agreement' })).toBe('low');
      expect(correctionsSystem.determineSeverity({ type: 'unknown' })).toBe('low');
    });
  });
  ...
  describe('Correction Interaction', () => {
    beforeEach(() => {
      ...
      const correctionId = correctionsSystem.generateCorrectionId(sampleCorrection);
    ...
    it('should hide other correction details when showing new one', () => {
      ...
      const details1 = document.getElementById(`correction-details-${correctionIds[0]}`);
      const details2 = document.getElementById(`correction-details-${correctionIds[1]}`);
  ...
  describe('Audio Playback', () => {
    ...
    it('should handle audio playback errors', async () => {
      ...
      const showNotificationSpy = vi.spyOn(correctionsSystem, 'showNotification');
      ...
      expect(showNotificationSpy).toHaveBeenCalledWith('Audio playback not available', 'error');
    });
  });
  ...
  describe('Practice Mode', () => {
    it('should increment practice count and update UI', () => {
      ...
      const showPracticePromptSpy = vi.spyOn(correctionsSystem, 'showPracticePrompt');
