...
**Problem**: TypeScript SDK generated by `openapi-typescript-codegen` includes `@ts-ignore` directives for Node.js Buffer fallback usage, but Biome linter was configured to reject these directives, causing API Lint workflow failures.
...
- [x] **Modified `.github/linters/biome.sdk.json`**: Added `"noTsIgnore": "off"` to allow `@ts-ignore` in generated SDK files
