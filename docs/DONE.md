...
**Problem**: Three TTS tests failing in CI environments due to path resolution differences and incorrect text
verification.

**Root Cause**:

- Tests expected bare command names like `"espeak-ng"` but CI environments return full paths like `"/usr/bin/espeak-ng"`
  from `shutil.which()`
...
**Impact**: All TTS tests now pass consistently across development and CI environments while maintaining security best
practices.

## 63) TypeScript SDK @ts-ignore Configuration Fix (2025-09-02) ✅

### SDK/Linter Synchronization Issue Resolution

**Problem**: TypeScript SDK generated by `openapi-typescript-codegen` includes `@ts-ignore` directives for Node.js
Buffer fallback usage, but Biome linter was configured to reject these directives, causing API Lint workflow failures.

**Root Cause**:

- Generated SDK code includes `@ts-ignore` for legitimate Buffer usage in Node.js environments
- Biome `noTsIgnore` rule was enabled for SDK files, treating generated directives as code quality violations
- Local development environment didn't need the directive but CI environment expected exact generator output

**Solution**: SDK-specific linting configuration to allow generated code patterns:

- [x] **Modified `.github/linters/biome.sdk.json`**: Added `"noTsIgnore": "off"` to allow `@ts-ignore` in generated SDK
      files
- [x] **Preserved code quality**: Main Biome configuration still enforces strict rules for handwritten code
- [x] **Generator compatibility**: SDK now matches exactly what `openapi-typescript-codegen` produces
- [x] **CI/local parity**: Both environments now accept the same generated SDK format
...
- ✅ **API Lint workflow now passes**: TypeScript SDK generation and linting work seamlessly in CI
