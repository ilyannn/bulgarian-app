...
class SetupVerifier:
    ...
    def check_python_version(self):
        ...
        if version >= (3, 11):
            ...
            self.print_warning(f"Python {version.major}.{version.minor}.{version.micro} works but 3.11+ is recommended")
        else:
            self.print_error(f"Python {version.major}.{version.minor}.{version.micro} is too old. Need 3.9+")
    ...
    def check_system_dependencies(self):
        ...
        if self.check_command("uv", ["--version"]):
            ...
            self.print_error("uv package manager not found. Install from: https://github.com/astral-sh/uv")
        ...
        if self.check_command("just", ["--version"]):
            ...
            self.print_error("just command runner not found. Install from: https://github.com/casey/just")
        
        # Check eSpeak NG
        if self.check_command("espeak-ng", ["--version"]) or self.check_command("espeak", ["--version"]):
            self.print_success("eSpeak NG text-to-speech is installed")
        else:
            self.print_error("eSpeak NG not found. Install with: apt-get install espeak-ng (Linux) or brew install espeak (macOS)")
        
        # Check Docker (optional)
        if self.check_command("docker", ["--version"]):
            self.print_success("Docker is installed (for security scanning)")
        else:
            self.print_warning("Docker not found (optional, needed for security scanning)")
    ...
    def check_python_dependencies(self):
        ...
        try:
            ...
            result = subprocess.run(["uv", "run", "python", "-c", "import sys; print('Python environment OK')"], 
            ...
            if result.returncode == 0:
                ...
                important_packages = [
                    "fastapi", "uvicorn", "faster-whisper", "pydantic", 
                    "websockets", "openai", "anthropic"
                ]
                
                for package in important_packages:
                    try:
                        result = subprocess.run(["uv", "run", "python", "-c", f"import {package}"], 
                                              capture_output=True, cwd=self.project_root)
                        if result.returncode == 0:
                            self.print_success(f"✓ {package}")
                        else:
                            self.print_warning(f"Package {package} may not be installed")
                    except:
    ...
    def check_javascript_dependencies(self):
        ...
        if package_json.exists():
            ...
            if node_modules.exists():
                ...
                try:
                    result = subprocess.run(["bun", "run", "--help"], 
                                          capture_output=True, cwd=self.project_root / "client")
                    if result.returncode == 0:
                ...
                self.print_error("JavaScript dependencies not installed. Run: just install")
        else:
    ...
    def check_environment_config(self):
        ...
        if env_file.exists():
            ...
            try:
                from config import validate_startup_environment, ConfigError
            ...
            self.print_warning("No .env file found. Copy .env.example to .env and configure it")
    ...
    def run_basic_tests(self):
        ...
        for recipe in recipes_to_test:
            try:
                ...
                result = subprocess.run(["just", recipe], capture_output=True, 
                                      text=True, cwd=self.project_root, timeout=60)
                if result.returncode == 0:
    ...
    def print_summary(self):
        ...
        if self.successes:
            print(f"{Colors.GREEN}✅ Successful checks: {len(self.successes)}{Colors.END}")
        ...
        if not self.issues:
            if not self.warnings:
                print(f"{Colors.GREEN}🎉 Perfect! Your setup is ready for development.{Colors.END}")
            else:
                print(f"{Colors.YELLOW}✅ Good! Setup is functional with minor warnings.{Colors.END}")
            return True
        elif len(self.issues) <= 2:
            print(f"{Colors.YELLOW}⚠️  Setup has some issues but may still work.{Colors.END}")
            return False
        else:
            print(f"{Colors.RED}❌ Setup has significant issues that need to be resolved.{Colors.END}")
            return False
