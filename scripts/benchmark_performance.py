...
class PerformanceBenchmark:
    ...
    async def benchmark_configuration(
        ...
        print(f"  Settings: VAD={config['vad_tail_ms']}ms, beam={config['beam_size']}, "
    ...
    def print_result_summary(self, result: Dict[str, Any]):
        ...
        print(f"\n{status} {config['name']}:")
        print(f"  Latency: {latency['mean']:.1f}ms (median: {latency['median']:.1f}ms, "
              f"p90: {latency['p90']:.1f}ms, p99: {latency['p99']:.1f}ms)")
        print(f"  Accuracy: {accuracy['mean']:.2%} (min: {accuracy['min']:.2%}, "
    ...
    def analyze_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i, result in enumerate(ranked[:3], 1):
            ...
            print(f"   Latency: {result['latency']['mean']:.1f}ms, "
        ...
        print(f"🎯 Most Accurate: {most_accurate['configuration']['name']} "
        ...
        if optimal['latency']['mean'] > 1500:
            ...
            print(f"   - Consider using a smaller ASR model")
            print(f"   - Optimize LLM response time")
            print(f"   - Pre-generate common TTS responses")
