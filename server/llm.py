...
class DummyProvider(ChatProvider):
    ...
    async def get_response(self, user_input: str, system_prompt: str, **kwargs) -> str:
        ...
        response_template = self.bulgarian_responses[self.response_count % len(self.bulgarian_responses)]
...
async def get_chat_response(user_input: str, provider: ChatProvider = None) -> str:
    ...
    try:
        response = await provider.get_response(user_input, BULGARIAN_COACH_SYSTEM_PROMPT)
        return response
    except Exception as e:
        print(f"Chat response error: {e}")
        # Fallback to dummy provider
        dummy_provider = DummyProvider()
        return await dummy_provider.get_response(user_input, BULGARIAN_COACH_SYSTEM_PROMPT)